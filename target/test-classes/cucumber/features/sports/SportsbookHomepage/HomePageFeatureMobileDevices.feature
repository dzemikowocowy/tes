...
Feature: Homepage in mobile
  ...
  Scenario Outline:  Go to home page from different pages through main icon
    ...
    And he clicks on home button in mobile navigation header
    Then home page is displayed
    Examples:
      |navigationoption|
      | in-play       |
  ...
  Scenario: Check carousel navigation
    Given the user navigates to WilliamHill homepage
    Then the mobile 'default' Carousel displayed with correct order

  Scenario Outline:Options in Football Carousel redirect to correct pages.
    Given the user is on the William Hill Home page
    Then the user select '<option>' from the footbal Carousel
    Examples:
      |option|
      |in-play|
      |football|
